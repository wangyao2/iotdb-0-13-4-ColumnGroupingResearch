...
public class TSServiceImpl implements TSIService.Iface {
    ...
    try {
      ...
      PhysicalPlan physicalPlan =
          serviceProvider
              .getPlanner()
              .parseSQLToPhysicalPlan(statement, session.getZoneId(), session.getClientVersion());
      // 在这里通过监测statement语句中是否包含autoalgined，手动给physical plan设置成为自动对齐序列
      if (physicalPlan instanceof InsertPlan) {
        if (containsAutoalign(statement)) {
          physicalPlan.setAutoAligned(true);
        }
      }
  ...
  public static boolean containsAutoalign(String statement) {
    String[] words = statement.split(" ");
    for (String word : words) {
      if (word.equals("autoaligned")) {
        return true;
      }
    }
    return false;
  ...
  /** Pyhton客户端的execute_non_query_statement会调用这个函数来插入数据，update statement can be: 1. select-into statement 2. non-query statement */
  @Override
