...
public class ApproximateFlushGroupingEngine {
  ...
  public static class ColumnGroupAppr {
    ...
    public void updateTimestampInfo(AlignedTVList dataList) {
      ...
      rangeMapCols.add(overlapGroup);//这个range看上去没有什么用处
    }
  ...
  public static void grouping(AlignedTVList dataList, List<IMeasurementSchema> schemaList) {
      ...
      for (int i = 0; i < timeSeriesNumber; i++) {
        ...
        if (bitmap == null) {
          ...
          for (int j = 0; j < dataList.getBitMaps().size(); j++) {
            if (dataList.getBitMaps().get(j) != null) {//找到第一个不是空的bitmap列表
              List<BitMap> bitMaps = dataList.getBitMaps().get(j);
              int size1ofAbitmap = bitMaps.get(0).getSize();//获取一个bitmap元素的长度，对应byte[]，对应bitmap内的属性
              int sizeBitmap = dataList.getBitMaps().get(j).size();//获取一个列的，bitmap有多长，对应序列有多长，可以对应时间戳长度
              for (int k = 0; k < sizeBitmap; k++) {//待解决问题，每次都会导致bitmap为0的数量增加
                bitmap.add(new BitMap(size1ofAbitmap, bytes.clone()));
              }
              break;//处理完一个序列后，就break直接处理下一个
            }
          }
          dataList.getBitMaps().set(i,bitmap);
        }
        columnGroups.add(
    ...
    String outputFile = "./time_costsApprFlush.csv";
  ...
  public static void outputGroupingResults(
          ArrayList<ColumnGroupAppr> columnGroups, List<IMeasurementSchema> schemaList) {
    String outputFile = "./grouping_results_Appr.csv";
    BufferedWriter out = null;
    try {
      out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(outputFile, true)));
      for (int i = 0; i < columnGroups.size(); i++) {
        for (int j = 0; j < columnGroups.get(i).columns.size() - 1; j++) {
          ...
          String measurement = schemaList.get(idx).getMeasurementId();
          out.write(measurement + ",");
        }
        int idx = columnGroups.get(i).columns.get(columnGroups.get(i).columns.size() - 1);
        out.write(schemaList.get(idx).getMeasurementId() + "\r\n");
      }
    } catch (Exception e) {
      e.printStackTrace();
    } finally {
