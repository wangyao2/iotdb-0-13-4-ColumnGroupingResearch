    ...
    for (IDeviceID deviceID : deviceIDList) {
      ...
      for (Map.Entry<String, IWritableMemChunk> iWritableMemChunkEntry : value.entrySet()) {
        ...
        if (series instanceof AutoAlignedWritableMemChunk) {
          ...
          WangsGroupingEngine.grouping(dataList, schemaList);//在一次的数据输入中，都做处理
        }

        if (series instanceof AlignedWritableMemChunk) {
          /** perform our grouping strategy* */
          List<IMeasurementSchema> schemaList =
                  ((AlignedWritableMemChunk) series).getSchemaList();
          AlignedTVList dataList = ((AlignedWritableMemChunk) series).getList();
          //FlushGroupingEngine.grouping(dataList, schemaList);
          //ApproximateFlushGroupingEngine.grouping(dataList, schemaList);
          WangsGroupingEngine.grouping(dataList, schemaList);
        }
      }
